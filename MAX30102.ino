...
//spo2_tabla aproximadamente:  -45.060*ratioAverage* ratioAverage + 30.354 *ratioAverage + 94.845 ;
  ...
  public:
    Waveform(void) {wavep = 0;}
      ...
      void record(int waveval) {
        waveval = waveval/8;         // escalamiento para entrar al byte 
        waveval += 128;              
      ...
      void scale() {
        ...
        uint8_t scale8 = (maxw-minw)/4 + 1;  //scale * 8 para preservar la precisión
        uint8_t index = wavep;
        for (int i=0; i<MAXWAVE; i++) {
...
void go_sleep() {
    ...
    sensor.off();
    delay(10);
    cbi(ADCSRA, ADEN);  //deshabilitar adc
    delay(10);
    pinMode(0,INPUT);
    pinMode(2,INPUT);
    set_sleep_mode(SLEEP_MODE_PWR_DOWN);     
    sleep_mode();  //dormir hasta presionar el botón
    // reseteo
    setup();
}
...
void draw_oled(int msg) {
    ...
    do{
    switch(msg){
        case 0:  oled.drawStr(10,0,F("Error"),1); 
                 break;
        case 1:  oled.drawStr(0,0,F("Colocar el"),2); 
                 oled.drawStr(25,18,F("dedo"),2);
                 break;

        case 2:  print_digit(86,0,beatAvg,' ');
                 oled.drawChar(30,0,'lpm',2);
                 oled.drawStr(0,3,F("PULSO: "),1);
                 oled.drawStr(11,17,F("SATURACIÓN: "),1);
                 print_digit(73,16,SPO2f,' ',3,2);
                 oled.drawChar(116,16,'%',2);
                 break;

        case 3:  oled.drawStr(25,15,F("AMACHAY"),2);
                 //oled.drawXBMP(6,8,16,16,heart_bits);
                 break;

        case 4:  oled.drawStr(28,12,F("Se apagará en"),1);
                 oled.drawChar(76,12,10-sleep_counter/10+'0');
                 oled.drawChar(82,12,'s');
                 break;
                 
        case 5:  oled.drawStr(0,0,F("Avg Pulso: "),1); 
                 print_digit(75,0,beatAvg);
                 oled.drawStr(0,15,F("Avg Saturación: "),1); 
...
long displaytime = 0; //Tiempo de la última actualización de la pantalla
bool led_on = false;


void loop()  {
    sensor.check();
    long now = millis();   //empezar conteo de este ciclo
    ...
    if (irValue<5000) {
        voltage = getVCC();
        checkbutton();
        draw_oled(sleep_counter<=50 ? 1 : 4); // si el dedo no presiona
        delay(200);
        ++sleep_counter;
        ...
        // actualizar la pantalla cada 50 ms si el dedo hacia abajo
